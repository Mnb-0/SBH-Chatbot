...
def get_rag_components(api_key):
    ...
    if not os.path.exists(db_path):
        raise FileNotFoundError(f"Database folder '{db_path}' not found! Run indexer.py first.")
    ...
    generator_llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0.3, groq_api_key=api_key)
    critic_llm = ChatGroq(model="llama-3.3-70b-versatile", temperature=0, groq_api_key=api_key)
...
def get_sector_filter(query, llm):
    ...
    """
    try:
        response = llm.invoke([HumanMessage(content=routing_prompt + f"\nQuery: {query}")])
        category = response.content.strip()
        valid_sectors = ["Real Estate", "Trading", "Contracting", "Industrial", "Services"]
...
def verify_response(critic_llm, context, answer):
    ...
    response = critic_llm.invoke([
        SystemMessage(content="You are a strict JSON-output machine. No conversational filler."),
    ...
    try:
        ...
        return {"is_hallucinated": True, "reason": "Auditor failed to output valid JSON. Flagged as hallucination for safety."}
